...
namespace hpx { namespace util
{
    namespace detail
    {
        ...
        T* begin_impl(T (&array)[N], long) HPX_NOEXCEPT
        {
            return &array[0];
        }
        ...
        template <typename T, std::size_t N>
        HPX_HOST_DEVICE HPX_CONSTEXPR HPX_FORCEINLINE
        T* end_impl(T (&array)[N], long) HPX_NOEXCEPT
        {
            return &array[N];
        }
        ...
        template <typename T, std::size_t N>
        HPX_HOST_DEVICE HPX_CONSTEXPR HPX_FORCEINLINE
        std::size_t size_impl(T const (&array)[N], long) HPX_NOEXCEPT
        {
            return N;
        }
        ...
        template <typename T, std::size_t N>
        HPX_HOST_DEVICE HPX_CONSTEXPR HPX_FORCEINLINE
        bool empty_impl(T const (&array)[N], long) HPX_NOEXCEPT
        {
            return false;
        }
        ...
        R begin_impl(C& c, long)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(c.begin()))
        {
            return c.begin();
        }
        ...
        R size_impl(C const& c, long)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(c.size()))
        {
            return c.size();
        }
        ...
        R empty_impl(C const& c, long)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(c.empty()))
        {
            return c.empty();
        }
        ...
        namespace range_impl
        {
            ...
            R begin_impl(C& c, int)
                HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(begin(c)))
            {
                return begin(c);
            }
            ...
            HPX_HOST_DEVICE HPX_CONSTEXPR HPX_FORCEINLINE
            R end_impl(C& c, int)
                HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(end(c)))
            {
                return end(c);
            }
        }
    ...
    namespace range_adl
    {
        ...
        HPX_HOST_DEVICE HPX_CONSTEXPR HPX_FORCEINLINE
        Iterator begin(C& c)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(detail::begin_impl(c, 0L)))
        {
        ...
        HPX_HOST_DEVICE HPX_CONSTEXPR HPX_FORCEINLINE
        Sentinel end(C& c)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(detail::end_impl(c, 0L)))
        {
        ...
        std::size_t size(C const& c)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(detail::size_impl(c, 0L)))
        {
        ...
        bool empty(C const& c)
            HPX_NOEXCEPT_IF(HPX_NOEXCEPT_EXPR(detail::empty_impl(c, 0L)))
        {
